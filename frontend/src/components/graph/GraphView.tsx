/* eslint-disable @typescript-eslint/no-explicit-any -- react-force-graph's internal
   node type uses `[others: string]: any`, and its ForceGraphMethods is incompatible
   with React 19's immutable RefObject. Strongly typing callbacks/ref causes
   contravariance errors. This is standard practice for d3 / force-graph / three.js. */
...
function GraphLoading() {
  return (
    <div className="flex items-center justify-center w-full h-full">
      <span className="text-sm text-[var(--zen-on-surface-variant)]">Loading graph engine…</span>
    </div>
  );
}

type ForceGraphNode = Record<string, any>;
...
export default function GraphView({ graphData }: GraphViewProps) {
  ...
  return (
    <div ref={containerRef} className="w-full h-[calc(100vh-4rem)] relative">
      {/* View toggle */}
