  ...
  return (
    <div className="flex-1 relative flex flex-col items-center justify-start pt-4 md:pt-8 overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center w-full px-6">
        ...
        >
          The Future of <em className="font-[family-name:var(--font-instrument-serif)]">Smarter</em>{" "}
          Note-Taking
        </motion.h1>
        ...
        >
          Capture your thoughts with intelligent notes that organize themselves, surface insights,
          and adapt to how you think &mdash; so you can focus on what matters most.
        </motion.p>
...
function DashboardPreview() {
  return (
    <div className="rounded-2xl overflow-hidden p-3 md:p-4 bg-card border border-border/50 shadow-2xl shadow-black/5">
      <div className="flex h-[420px] rounded-xl overflow-hidden bg-background text-foreground select-none pointer-events-none text-[11px]">
        <Sidebar />
        <MainContent />
      </div>
    </div>
...
function Sidebar() {
  ...
  const workflowItems = ["Workflows", "Tags", "Notifications", "Settings"];
      ...
      <div className="flex-1 space-y-0.5">
        {mainItems.map((item) => (
          ...
          >
            ...
            {item.chevron && <ChevronRight className="h-3 w-3 shrink-0" />}
          </div>
        ))}
        ...
        <div className="pt-2 mt-2 border-t border-border/60">
          ...
          {workflowItems.map((label) => (
            ...
            >
              {label === "Tags" && <Tag className="h-3.5 w-3.5 shrink-0" />}
              {label === "Notifications" && <Bell className="h-3.5 w-3.5 shrink-0" />}
              {label === "Settings" && <Settings className="h-3.5 w-3.5 shrink-0" />}
              {label === "Workflows" && <FileEdit className="h-3.5 w-3.5 shrink-0" />}
              <span>{label}</span>
            </div>
          ))}
        </div>
...
function MainContent() {
  const actionButtons = ["New Note", "Search", "Organize", "Export", "Share"];
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 bg-secondary/20 p-4 overflow-hidden">
        <div className="text-[12px] font-semibold mb-3">Welcome back, Jane</div>
        ...
        <div className="flex gap-3 mb-4">
          <div className="flex-1 basis-0 rounded-xl border border-border/60 bg-card p-3">
            ...
            <div className="text-[18px] font-semibold tracking-tight">1,247</div>
            <div className="text-[9px] text-muted-foreground mb-2">notes across all notebooks</div>
            <StatsChart />
          </div>

          <div className="flex-1 basis-0 rounded-xl border border-border/60 bg-card p-3">
            <div className="flex items-center justify-between mb-3">
              <div className="text-[10px] font-semibold">Notebooks</div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {recentNotes.map((note) => (
                <tr key={note.title} className="border-b border-border/20 last:border-0">
                  <td className="px-3 py-2 text-[11px] font-medium">{note.title}</td>
                  <td className="px-3 py-2 text-[11px] text-muted-foreground">{note.folder}</td>
                  <td className="px-3 py-2 text-[11px] text-muted-foreground flex items-center gap-1">
                    <Clock className="h-2.5 w-2.5" />
                    {note.date}
                  </td>
                  <td className="px-3 py-2 text-right">
                    <span className={`text-[10px] font-medium ${note.statusColor}`}>
                      {note.status}
                    </span>
                  </td>
                </tr>
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2.5 border-b border-border/60">
      <div className="flex items-center gap-3">
        ...
        <span className="text-[11px] text-muted-foreground">All Notes</span>
      </div>
      <div className="flex items-center gap-3">
